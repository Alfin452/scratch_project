<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto">
        ...
        {{-- === EDIT FORM PILIHAN GANDA === --}}
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden" x-data="editMultipleChoice({{ json_encode($task->content) }})">
            <div class="flex items-center gap-3 p-6 border-b border-gray-200 dark:border-gray-700 bg-pink-50 dark:bg-pink-900/20">
                ...
                <div>
                    <h3 class="font-bold text-gray-800 dark:text-white">Edit Soal Pilihan Ganda</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400">Edit pertanyaan dan opsi jawaban</p>
                </div>
            </div>
    ...
    <script>
        ...
        function editMultipleChoice(content) {
            let parsedQs = content?.questions || [
                { text: '', options: ['', '', '', ''], correctOption: null }
            ];
            
            // Beri ID unik agar AlpineJS loop re-render lancar
